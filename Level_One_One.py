...
#GET HOST MOVE
def CompMove_First(board, comp_letter):
    if comp_letter == 'X':
        player_letter = 'O'
    else:
        player_letter = 'X'
    ...
    move = RandomMove(board, [1, 3, 5, 7, 9])
    if move != None:
        return move

    return RandomMove(board, [2, 4, 6, 8])

def CompMove_Second(board, comp_letter):
    if comp_letter == 'X':
        player_letter = 'O'
    else:
        player_letter = 'X'

    # OPPONENT FIRST MOVE IS CENTER, SECURE ONE CORNER
    if board[5] == player_letter:
        move = RandomMove(board, [1, 3, 7, 9])
        if move != None:
            return move

    # OPPONENT FIRST MOVE IS CORNER, SECURE MIDDLE
    elif ((board[1] == player_letter) or
              (board[3] == player_letter) or
              (board[7] == player_letter) or
    ...
    move = RandomMove(board, [1, 2, 3, 4, 5, 6, 7, 8, 9])
    if move != None:
        return move
#=================Level 1.5==================#
    ...
    while (True):
        ...
        while (ongoingGame):
            if turn == 'PLAYER / COMPUTER':
                ...
                move = CompMove_First(board, comp_letter)
